...
export function GlobalKpis({ totalTimeMinutes, activeDays, streak, loading }: GlobalKpisProps) {
  const t = useTranslations('GlobalKpis');
  ...
  const getStreakMessage = (count: number) => {
    if (count >= 30) return t('streakMessages.epic');
    if (count >= 14) return t('streakMessages.incredible');
    return t('streakMessages.great');
  };

  return (
    <div data-onboarding="stats-01" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '1rem' }}>
      ...
      <div style={{ background: 'var(--surface)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.6rem' }}>
          ...
          <span style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--muted)' }}>
            {t('totalTime')}
          </span>
        </div>
      ...
      <div style={{ background: 'var(--surface)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.04)' }}>
        ...
        <p style={{ color: 'var(--text)', fontSize: '2.4rem', margin: 0, fontFamily: 'Bebas Neue, sans-serif', lineHeight: 1 }}>
          {activeDays}
          <span style={{ fontSize: '0.9rem', color: 'var(--muted)', fontFamily: 'DM Sans, sans-serif', fontWeight: 600, marginLeft: '0.3rem' }}>
            {t('daysUnit', { count: activeDays })}
          </span>
        </p>
      </div>
