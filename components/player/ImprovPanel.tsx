...
export function ImprovPanel() {
  ...
  const suggestedScales = useMemo(() => {
    ...
    if (isMinor) {
      if (SCALES['aeolian']) recommendations.push(`${SCALES['aeolian'].name} de ${rootNote}`);
      if (SCALES['pentatonic-minor']) recommendations.push(`${SCALES['pentatonic-minor'].name} de ${rootNote}`);
      if (progressionObj.roman.includes('V') && SCALES['harmonic-minor']) {
        recommendations.push(`${SCALES['harmonic-minor'].name} de ${rootNote}`);
      }
    } else {
      if (SCALES['ionian']) recommendations.push(`${SCALES['ionian'].name} de ${rootNote}`);
      if (SCALES['pentatonic-major']) recommendations.push(`${SCALES['pentatonic-major'].name} de ${rootNote}`);
      const relMinorRoot = CHROMATIC_NOTES[(CHROMATIC_NOTES.indexOf(rootNote) + 9) % 12];
      if (SCALES['aeolian']) recommendations.push(`${SCALES['aeolian'].name} de ${relMinorRoot}`);
    }

    return recommendations.join(' o ');
  }, [progressionObj, rootNote]);
    ...
    <div style={{ 
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        <div style={{ background: 'var(--surface)', padding: '2rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)', textAlign: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <h3 style={{ color: 'var(--gold)', margin: 0, fontSize: '1.2rem' }}>Generador de Ideas</h3>
            <select 
              value={rootNote} 
            ...
            >
              {CHROMATIC_NOTES.map(note => (
                <option key={note} value={note}>Tonalidad: {note} {progressionObj.isMinorStart ? 'Menor' : 'Mayor'}</option>
              ))}
            </select>
          </div>
          ...
          <div style={{ background: 'rgba(220,185,138,0.1)', border: '1px solid rgba(220,185,138,0.2)', padding: '1rem', borderRadius: '8px', marginBottom: '1.5rem', textAlign: 'left' }}>
            ...
            <span style={{ fontSize: '0.9rem', color: 'var(--text)' }}>
              Prueba a improvisar usando: <strong>{suggestedScales}</strong>
            </span>
          </div>
        ...
        <div style={{ background: 'var(--surface)', padding: '2rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
          <h3 style={{ color: 'var(--gold)', margin: '0 0 1rem 0', fontSize: '1.2rem' }}>Backing Track (YouTube)</h3>
          <input 
            type="text" 
            value={ytLink}
            onChange={(e) => setYtLink(e.target.value)}
            placeholder="Pega el enlace de un backing track de YouTube..."
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <div style={{ background: 'var(--surface)', padding: '2rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', height: '100%', minHeight: '500px' }}>
          <h3 style={{ color: 'var(--gold)', margin: '0 0 1rem 0', fontSize: '1.2rem' }}>Bloc de Notas de Composición</h3>
          <textarea 
            value={notes}
