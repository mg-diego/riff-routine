...
export default function ImportExercisesPage() {
    ...
    const validateTechnique = (techString: string): string | undefined => {
        if (!techString || techString.trim() === '') {
            return 'Debes seleccionar al menos una técnica.';
        }
        const inputTechs = techString.split(',').map(t => t.trim()).filter(Boolean);
        if (inputTechs.length === 0) {
            return 'Debes seleccionar al menos una técnica.';
        }
        const invalidTechs = inputTechs.filter(t => !TECHNIQUES.includes(t));
        if (invalidTechs.length > 0) {
            return `Técnica inválida: ${invalidTechs.join(', ')}`;
        }
        return undefined;
    };
    ...
    const parseText = () => {
        ...
        if (extracted.length === 0) {
            setGlobalError("No se encontraron ejercicios válidos para analizar.");
            setParsedData([]);
            return;
        }
    ...
    const handleImport = async () => {
        ...
        if (hasErrors) {
            setGlobalError("Corrige los errores en las filas marcadas antes de continuar.");
            return;
        }
                ...
                {parsedData.length === 0 ? (
                    <>
                        <div style={{ marginBottom: '1.5rem' }}>
                            <p style={{ color: 'var(--muted)', margin: '0 0 0.5rem', fontSize: '0.9rem' }}>
                                Pega tu lista separando el título, la técnica y los BPM objetivo con dos guiones (--). Un ejercicio por línea.
                            </p>
                            <div style={{ background: 'rgba(255,255,255,0.03)', padding: '1rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.08)' }}>
                                ...
                                <code style={{ color: 'var(--text)', background: '#111', padding: '0.3rem 0.6rem', borderRadius: '4px', fontSize: '0.85rem' }}>Nombre -- Técnica1, Técnica2 -- Minutos (opcional)</code><br /><br />
                                <code style={{ color: 'var(--muted)', fontSize: '0.85rem' }}>Araña 1234 -- Alternate Picking -- 120</code><br />
                                <code style={{ color: 'var(--muted)', fontSize: '0.85rem' }}>Escala Menor -- Legato, Tapping -- 100</code>
                            </div>
                    ...
                    <>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                            <h3 style={{ color: 'var(--text)', margin: 0 }}>Revisar y Corregir ({parsedData.length})</h3>
                            <button
                                ...
                                return (
                                    <div key={ex.id} style={{
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                                            <label style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Título *</label>
                                            <input
                                                type="text"
                                                value={ex.title}
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', position: 'relative' }}>
                                            <label style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Técnica *</label>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                                            <label style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>BPM</label>
                                            <input
                                                type="number"
                                                value={ex.bpm_goal}
