...
export default function RoutineDetailsPage() {
  ...
  // Snapshot del último estado guardado en DB — para detectar cambios
  ...
  // true cuando el estado local difiere del snapshot guardado
  ...
  const fetchData = useCallback(async () => {
    ...
    if (routineError || !routineData) { setError('No se pudo cargar la rutina.'); setLoading(false); return; }
  ...
  // ── Actualizar nombre / descripción de la rutina ──────────────────────────
  ...
  // ── Guardar todos los cambios pendientes en DB ─────────────────────────────
  ...
  // ── Mutaciones locales (sin tocar DB hasta "Guardar cambios") ──────────────
  ...
  const handleRemoveExercise = async (id: string) => {
    ...
    else setSavedSnapshot(prev => {
      // actualizar snapshot también para que este cambio no cuente como "unsaved"
      const updated = routineExercises.filter(e => e.id !== id);
      return snapshotFrom(updated);
    });
  };

  // Solo actualiza estado local — no va a DB hasta que el user pulsa "Guardar"
